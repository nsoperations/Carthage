...
@test "carthage copy-frameworks with --auto option should work for all an app target, a unit test target, a UI test target and produce a valid xcarchive file" {
    ...
    run xcodebuild clean build-for-testing -scheme CarthageCopyFrameworksFixture -sdk iphonesimulator -destination "name=iPhone 8" CARTHAGE_AUTO_COPY_FRAMEWORKS="1"
    [ "$status" -eq 0 ]
