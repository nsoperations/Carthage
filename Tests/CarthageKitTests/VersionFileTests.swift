...
class VersionFileTests: XCTestCase {
	func testShouldReadAVersionFileCorrectly() {
	...
	func testShouldWriteAndReadBackAVersionFileCorrectly() {
	...
	func testShouldEncodeAndDecodeCorrectly() {
		...
		expect(iOSCache?[0].swiftToolchainVersion) == "4.2 (swiftlang-1000.11.37.1 clang-1000.11.45.1)"
		expect(iOSCache?[0].isSwiftFramework ?? false) == true
    ...
    func validate(file: VersionFile, matches: Bool, platform: Platform, commitish: String, configuration: String = "Debug", hashes: [String?],
        ...
        _ = file.satisfies(platform: platform, commitish: commitish, sourceHash: nil, configuration: configuration, resolvedDependenciesHash: nil, hashes: hashes, swiftVersionMatches: swiftVersionMatches)
	...
	func testShouldDoProperValidationChecks() {
		guard let versionFileURL = Bundle(for: type(of: self)).url(forResource: "TestVersionFile", withExtension: "") else {
